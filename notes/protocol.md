...
Desktop Chromium prefers native Web Serial. [Chrome 148 added Web Serial on Android](https://developer.chrome.com/blog/chrome-148-beta#web_serial_api_support_on_android), but its native serial chooser excluded the badge in a field report, so Android deliberately uses the WebUSB CDC adapter even when both APIs exist. The statically bundled `web-serial-polyfill` 1.0.15 keeps `requestDevice()` directly in the connection tap's transient user activation. The chooser filter is only exact runtime VID:PID `1d50:6198`; after selection, the adapter requires and claims CDC control class `0x02` and data class `0x0a`, selects configuration 1 when necessary, sends `SET_LINE_CODING` (`0x20`) for 1,000,000-baud 8-N-1, asserts DTR with `SET_CONTROL_LINE_STATE` (`0x22`), and moves bytes through the data interface's bulk endpoints. It does not claim the badge's HID functions. This describes source behavior, not completed Android hardware validation.
...
- Normal commands have an echo deadline of at most 4 seconds. Final image and BIO chunks get a separate 20-second response deadline after the echo because those paths persist the assembled payload before returning `SUCCESS`; ordinary chunks only update volatile staging memory. Confirmation-sweep frames use the same 20-second ceiling for both echo and response because the original persistence call may still be blocking the shell.
- Timeout diagnostics record whether the deadline expired before the exact command echo or while waiting for its response. Payloads remain redacted.
...
Image and BIO uploads allow at most two full transaction passes. A connected, non-aborted, non-deterministic failure in the first pass waits 500 ms, clears staging state, and restarts from chunk zero. Validation/type errors, allowlist blocks, explicit firmware rejection, cancellation, and disconnect do not restart. Non-final chunks accept only `OK`. The initial final chunk accepts `SUCCESS` or `OK`; only `SUCCESS` completes the transfer, while `OK` starts confirmation immediately instead of waiting for a timeout.

A connected final-chunk timeout or final `OK` gets one bounded, no-clear confirmation sweep before any full restart. The app replays chunks zero through final, accepts `OK` or `SUCCESS`, and stops immediately at the first `SUCCESS`. Starting with chunk zero avoids confusing a late echo from the original final command with the recovery command. Any recovery frame can complete persistence, so every recovery response gets the 20-second commit deadline and no blind same-command retry. This converges whether the original final frame was lost, committed with a lost reply, or encountered partial staging, and a confirmed `SUCCESS` clears firmware staging. A disconnect never enters this path.
