...
- The shell's exact `[console] <command>` echo arms the pending response matcher. Blank lines, mismatched echoes, and acknowledgement-looking lines received before that exact echo are ignored. Other Xous output after the echo is retained in the command log, but only an exact state-specific acknowledgement resolves the command. `ERR` or `ERR ...` rejects it.
